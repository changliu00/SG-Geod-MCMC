...
public:
	...
	GSGNHT(Model &model, vector<EigenVM> &samples, ER_C StoGradPtnEnergy, int D, int L = 1, double gamma = 0.01, double alpha = 0.01, double gradVariance = 0):
		model(model), samples(samples), StoGradPtnEnergy(StoGradPtnEnergy), L(L), 
		eps(sqrt(gamma/D)), sd(sqrt(2*alpha - gradVariance*eps*eps)), 
		var(samples.end() - 1), vlc(var->rows(), var->cols()), grad(var->rows(), var->cols()), noise(var->rows(), var->cols()), 
		xi(alpha/eps), dim(var->size()) {
	...
	void Sample(int moreN) { // "samples.back()" should not be changed between two calls of this function!
		...
		for(int i=0; i<moreN; i++) {
			...
			xi += (vlc.squaredNorm()/dim - 1)*eps*0.5;
			damper = exp(-xi*eps*0.5);
			StoGradPtnEnergy(model, grad);
			vlc = damper * (damper*vlc - eps*grad + sd*randN(noise));
			this->Projection();
			for(int j=1; j<L; j++) {
				this->GeodFlowUpdate();
				xi += (vlc.squaredNorm()/dim - 1)*eps;
				damper = exp(-xi*eps*0.5);
				StoGradPtnEnergy(model, grad);
				vlc = damper * (damper*vlc - eps*grad + sd*randN(noise));
				this->Projection();
...
class GSGNHT_spheres : public GSGNHT<Model, MatrixXd>
{
public:
	typedef function<void(const Model&, Ref<MatrixXd>)> ER_C;
	GSGNHT_spheres(Model &model, vector<MatrixXd> &samples, ER_C StoGradPtnEnergy, int D, int L = 1, double gamma = 0.01, double alpha = 0.01, double gradVariance = 0): GSGNHT<Model, MatrixXd>(model, samples, StoGradPtnEnergy, D, L, gamma, alpha, gradVariance) { this->Projection(); }
private:
	using GSGNHT<Model, MatrixXd>::var;
	using GSGNHT<Model, MatrixXd>::vlc;
	using GSGNHT<Model, MatrixXd>::eps;
...
template<class Model>
class GSGNHT_simplex : public GSGNHT<Model, VectorXd>
{
public:
	typedef function<void(const Model&, Ref<VectorXd>)> ER_C;
	GSGNHT_simplex(Model &model, vector<VectorXd> &samples, ER_C StoGradPtnEnergy, int D, int L = 1, double gamma = 0.01, double alpha = 0.01, double gradVariance = 0): GSGNHT<Model, VectorXd>(model, samples, StoGradPtnEnergy, D, L, gamma, alpha, gradVariance) { this->Projection(); }
private:
	using GSGNHT<Model, VectorXd>::var;
	using GSGNHT<Model, VectorXd>::vlc;
	using GSGNHT<Model, VectorXd>::eps;
...
template<class Model>
class GSGNHT_Stiefel : public GSGNHT<Model, MatrixXd>
{
public:
	typedef function<void(const Model&, Ref<MatrixXd>)> ER_C;
	GSGNHT_Stiefel(Model &model, vector<MatrixXd> &samples, ER_C StoGradPtnEnergy, int D, int L = 1, double gamma = 0.01, double alpha = 0.01, double gradVariance = 0): GSGNHT<Model, MatrixXd>(model, samples, StoGradPtnEnergy, D, L, gamma, alpha, gradVariance) { this->Projection(); }
private:
	using GSGNHT<Model, MatrixXd>::var;
	using GSGNHT<Model, MatrixXd>::vlc;
	using GSGNHT<Model, MatrixXd>::eps;
