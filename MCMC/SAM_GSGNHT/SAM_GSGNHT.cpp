...
// SAM_GSGNHT
SAM_GSGNHT::SAM_GSGNHT(const ParamManager &pm): SAM_Base(pm), S(pm.i("S"))
{
	...
	// write settings(.m)
	pm.Print(dirname+"settings", {"SAM_GSGNHT"}, true);
	pm.Printm(dirname+"settings.m", {"SAM_GSGNHT"}, true, dirname.substr(0, dirname.size()-1)+"_settings_GSGNHT");
}
SAM_GSGNHT::SAM_GSGNHT(const string &dirname, const ParamManager &pm): SAM_Base(For::re, dirname, pm), S(pm.i("S"))
{
	thetaSamples.resize(S);
	for(int s=0; s<S; s++) thetaSamples[s].reserve(thBnin+thN);
	batchIds.resize(S);
...
void SAM_GSGNHT::InitTheta(void)
{
...
void SAM_GSGNHT::Sample(const string &tsDirname)
{
	...
	if(purpose == For::tr) {
		...
		cerr << "ERROR in SAM_GSGNHT::Sample: wrong purpose!" << endl; throw;
	}
	SAM_Eval &tsModel = (*ptr_tsModel);
	const int N = pm.i("ts_N");
	GSGNHT_spheres<SAM_GSGNHT> gsgnhtBeta((*this), betaSamples, &SAM_GSGNHT::StoGradU_beta, D, pm.i("btL"), pm.d("btGm"), pm.d("btAl"));
...
#pragma omp parallel for
		for(int s=0; s<S; s++) {
			...
			GMC_simplex<SAM_GSGNHT> gmcThetas( (*this), thetaSamples[s], bind(&SAM_GSGNHT::U_thetas, _1, s), bind(&SAM_GSGNHT::GradU_thetas, _1, _2, s), pm.i("thL"), pm.d("thEps") );
...
void SAM_GSGNHT::WriteIncream(int pos)const
{
...
void SAM_GSGNHT::StoGradU_beta(Ref<MatrixXd> grad)const
{
	grad.setZero();
#pragma omp parallel for
...
double SAM_GSGNHT::U_thetas(int s)const
{
...
void SAM_GSGNHT::GradU_thetas(Ref<VectorXd> grad, int s)const
{
